...
class TokenBucketRateLimiter:
    ...
    def __init__(self, rate_per_minute: float, bucket_size: Optional[int] = None):
        ...
        logger.info(f"Initialized rate limiter: {rate_per_minute} req/min, bucket size: {self.bucket_size}")
...
class RateLimiterManager:
    ...
    def get_limiter(self, api_name: str, rate_per_minute: float) -> TokenBucketRateLimiter:
        """
        ...
        with self.lock:
            if api_name not in self.limiters:
                ...
                logger.info(f"Created rate limiter for {api_name}: {rate_per_minute} req/min")
