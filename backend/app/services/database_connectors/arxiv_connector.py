...
class ArxivConnector(DatabaseConnector):
    ...
    async def search(self, query: SearchQuery) -> List[SearchResult]:
        ...
        try:
            ...
            logger.info(
                f"ArXiv search returned {len(results)} results for query: {query.query}"
            )
            return results
    ...
    def _build_arxiv_query(self, query: SearchQuery) -> str:
        ...
        if query.date_range:
            ...
            if "start" in query.date_range:
                base_query += (
                    f" AND submittedDate:[{query.date_range['start']}0101 TO *]"
                )
    ...
    def _normalize_result(self, raw_result) -> SearchResult:
        ...
        try:
            ...
            result = SearchResult(
                ...
                authors=authors,
                abstract=self._clean_text(raw_result.summary)
                ...
                venue="arXiv",
                doi=raw_result.doi
                if hasattr(raw_result, "doi") and raw_result.doi
                else None,
